...
class ChatAccessRequestSerializer(serializers.ModelSerializer):
    ...
    def validate_email(self, value):
        existing_request = (
            ChatAccessRequest.objects.filter(email=value).order_by("-criado_em").first()
        )
        if existing_request:
            ...
            if status == "pendente":
                ...
                raise serializers.ValidationError(
                    "Este e-mail já foi aprovado. Você já tem acesso."
                )
            elif status == "recusado":
                raise serializers.ValidationError(
                    "Este e-mail já foi recusado. Entre em contato com o suporte."
                )
        return value
